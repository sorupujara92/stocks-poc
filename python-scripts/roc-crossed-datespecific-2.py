      ...
      with open("files/"+row[0]+".csv", mode='r') as infile:
          ...
          for i in q:
              ...
              if(loop<n-4 and len(data_line)>11):
                ...
                if(data_line[1]==args.date and len(data_line)>11 and float(data_line[7])>(float(data_line[10])*1.25) and float(data_line[6])>(float(data_line[9]))  and row[0] in stocksector and row[0] not in finalstocks):
                    FACTOR = float(data_line[5])/float(data_line[6])
                    OPENWITHFACTOR = float(data_line[2])/FACTOR
                    CLOSEWITHFACTOR = float(data_line[6])
                    if(CLOSEWITHFACTOR-OPENWITHFACTOR > float(data_line[11])*1.3):
